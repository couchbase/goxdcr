...
//    srv-host=_couchbase._tcp.xdcr.local,xdcr.testdomain,9001
...
//
//    _couchbase._tcp.xdcr.local	service = 0 0 9001 xdcr.testdomain.
var dnsSrvHostname string = "xdcr.local"

// NOTE - SRV entries for "name" will end with a .
var targetName string = "xdcr.testdomain."
...
func TestDNSHelper(t *testing.T) {
	...
	assert.Equal(targetName, addrs[0].Target)
	...
	assert.Equal(targetName, entries[0].Target)
	assert.Equal(uint16(9001), addrs[0].Port)
}
