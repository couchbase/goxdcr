...
func (top_detect_svc *TopologyChangeDetectorSvc) handleSourceTopologyChange(vblist_supposed []uint16, number_of_source_nodes int, err_in error) error {
	...
	// The following 2 lines are introduced as part of a future release, but is needed for calculations below
...
func (top_detect_svc *TopologyChangeDetectorSvc) handleTargetTopologyChange(diff_vb_list []uint16, target_vb_server_map map[uint16]string, err_in error) error {
	...
	top_detect_svc.logger.Infof("TopologyChangeDetectorSvc for pipeline %v handleTargetTopologyChange(%v mod %v) completed", top_detect_svc.mainPipelineTopic, top_detect_svc.targetMonitorCnt, top_detect_svc.logFrequency)
	return nil
}
