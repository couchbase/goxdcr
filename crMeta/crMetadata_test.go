...
func TestConstructCustomCRXattrForSetMeta(t *testing.T) {
	...
	pos, _, err := ConstructXattrFromHlvForSetMeta(meta, 0, composer, 0)
	assert.Nil(err)
...
func setMetaPruningWithNoNewChange(t *testing.T) {
	...
	assert.Nil(err)
	composer := base.NewXattrRawComposer(body)
	pos, _, err := ConstructXattrFromHlvForSetMeta(meta, 5*time.Second, composer, 0)
	assert.Nil(err)
	...
	composer = base.NewXattrRawComposer(body)
	pos, _, err = ConstructXattrFromHlvForSetMeta(meta, 3*time.Second, composer, 0)
	assert.Nil(err)
	assert.Contains(string(body[4:pos]), fmt.Sprintf("{\"cvCas\":\"%s\",\"src\":\"Cluster1\",\"ver\":\"%s\",", cvHex, cvHex))
...
func setMetaPruningWithNewChange(t *testing.T) {
	...
	assert.Nil(err)
	composer := base.NewXattrRawComposer(body)
	pos, _, err := ConstructXattrFromHlvForSetMeta(meta, 6*time.Second, composer, 0)
	assert.Nil(err)
	...
	assert.Nil(err)
	composer = base.NewXattrRawComposer(body)
	pos, _, err = ConstructXattrFromHlvForSetMeta(meta, 4*time.Second, composer, 0)
	assert.Nil(err)
	assert.Contains(string(body[4:pos]), fmt.Sprintf("{\"cvCas\":\"%s\",\"src\":\"SourceCluster\",\"ver\":\"%s\",", newCvHex, newCvHex))
...
func setMetaPruningWithMvNewChange(t *testing.T) {
	...
	assert.Nil(err)
	composer := base.NewXattrRawComposer(body)
	pos, _, err := ConstructXattrFromHlvForSetMeta(meta, 7*time.Second, composer, 0)
	assert.Nil(err)
	newCvHex := base.Uint64ToHexLittleEndian(cas)
	assert.Contains(string(body[4:pos]), fmt.Sprintf("{\"cvCas\":\"%s\",\"src\":\"SourceCluster\",\"ver\":\"%s\",", newCvHex, newCvHex))
