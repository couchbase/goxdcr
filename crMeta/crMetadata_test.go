...
func TestConstructCustomCRXattrForSetMeta(t *testing.T) {
	...
	pos, _, _, err := ConstructXattrFromHlvForSetMeta(meta, 0, composer, 0)
	assert.Nil(err)
...
func setMetaPruningWithNoNewChange(t *testing.T) {
	...
	assert.Nil(err)
	composer := base.NewXattrRawComposer(body)
	pos, _, _, err := ConstructXattrFromHlvForSetMeta(meta, 5*time.Second, composer, 0)
	assert.Nil(err)
	...
	composer = base.NewXattrRawComposer(body)
	pos, _, _, err = ConstructXattrFromHlvForSetMeta(meta, 3*time.Second, composer, 0)
	assert.Nil(err)
	assert.Contains(string(body[4:pos]), fmt.Sprintf("{\"cvCas\":\"%s\",\"src\":\"Cluster1\",\"ver\":\"%s\",", cvHex, cvHex))
...
func setMetaPruningWithNewChange(t *testing.T) {
	...
	assert.Nil(err)
	composer := base.NewXattrRawComposer(body)
	pos, _, _, err := ConstructXattrFromHlvForSetMeta(meta, 6*time.Second, composer, 0)
	assert.Nil(err)
	...
	assert.Nil(err)
	composer = base.NewXattrRawComposer(body)
	pos, _, _, err = ConstructXattrFromHlvForSetMeta(meta, 4*time.Second, composer, 0)
	assert.Nil(err)
	assert.Contains(string(body[4:pos]), fmt.Sprintf("{\"cvCas\":\"%s\",\"src\":\"SourceCluster\",\"ver\":\"%s\",", newCvHex, newCvHex))
...
func setMetaPruningWithMvNewChange(t *testing.T) {
	...
	assert.Nil(err)
	composer := base.NewXattrRawComposer(body)
	pos, _, _, err := ConstructXattrFromHlvForSetMeta(meta, 7*time.Second, composer, 0)
	assert.Nil(err)
	newCvHex := base.Uint64ToHexLittleEndian(cas)
	assert.Contains(string(body[4:pos]), fmt.Sprintf("{\"cvCas\":\"%s\",\"src\":\"SourceCluster\",\"ver\":\"%s\",", newCvHex, newCvHex))
...
func TestMergeMeta(t *testing.T) {
	...
	* 1. New at both source and target. Make sure we have MV but not PV.
	...
	assert.Nil(err)
	mvlen, _, err := VersionMapToDeltasBytes(mergedMeta.GetHLV().GetMV(), mv, 0, nil, 0)
	assert.Nil(err)
	pvlen, _, err := VersionMapToDeltasBytes(mergedMeta.GetHLV().GetPV(), pv, 0, nil, 0)
	assert.Nil(err)
...
func TestXattrToVersionMap(t *testing.T) {
	...
	pos, _, err := VersionMapToDeltasBytes(pv, body, 0, nil, 0)
	assert.Nil(t, err)
