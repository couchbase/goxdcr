...
func TestConstructCustomCRXattrForSetMeta(t *testing.T) {
	...
	meta, err = NewMetadataForTest(docKey, sourceClusterId, ver+1000, 1, []byte("0x0b0085b25e8d1416"), []byte("Cluster4"), []byte("0x0b0085b25e8d1416"),
		[]byte("{\"Cluster1\":\"0x0000f8da4d881416\",\"Cluster2\":\"0x1500f8da4d881416\",\"Cluster3\":\"0x2000f8da4d881416\"}"), nil)
	assert.Nil(err)
	composer = base.NewXattrRawComposer(body)
	pos, _, err = ConstructXattrFromHlvForSetMeta(meta, 0, composer)
	assert.Nil(err)
	assert.Contains(string(body[4:pos]), "_vv\x00{\"cvCas\":\"0xf30385b25e8d1416\",\"src\":\"SourceCluster\",\"ver\":\"0xf30385b25e8d1416\",\"pv\":")
	assert.Contains(string(body[4:pos]), "\"Cluster1\":\"0x0000f8da4d881416\"")
	assert.Contains(string(body[4:pos]), "\"Cluster2\":\"0x1500f8da4d881416\"")
	assert.Contains(string(body[4:pos]), "\"Cluster3\":\"0x2000f8da4d881416\"")
	assert.Contains(string(body[4:pos]), "\"Cluster4\":\"0x0b0085b25e8d1416\"")
	assert.NotContains(string(body[4:pos]), "mv")
	assert.Equal(uint32(pos-4), binary.BigEndian.Uint32(body[0:4]))

	// Test 4: New change (cas=ver+1000) with existing XATTR (mv):
	meta, err = NewMetadataForTest(docKey, sourceClusterId, ver+1000, 1, []byte("0x0b0085b25e8d1416"), []byte("Cluster4"), []byte("0x0b0085b25e8d1416"),
		nil, []byte("{\"Cluster1\":\"0x0000f8da4d881416\",\"Cluster2\":\"0x1500f8da4d881416\",\"Cluster3\":\"0x2000f8da4d881416\"}"))
	assert.Nil(err)
	composer = base.NewXattrRawComposer(body)
	pos, _, err = ConstructXattrFromHlvForSetMeta(meta, 0, composer)
	assert.Nil(err)
	assert.Contains(string(body[0:pos]), "_vv\x00{\"cvCas\":\"0xf30385b25e8d1416\",\"src\":\"SourceCluster\",\"ver\":\"0xf30385b25e8d1416\",\"pv\":")
	assert.Contains(string(body[0:pos]), "\"Cluster1\":\"0x0000f8da4d881416\"")
	assert.Contains(string(body[0:pos]), "\"Cluster2\":\"0x1500f8da4d881416\"")
	assert.Contains(string(body[0:pos]), "\"Cluster3\":\"0x2000f8da4d881416\"")
	assert.NotContains(string(body[0:pos]), "mv")
	assert.Equal(uint32(pos-4), binary.BigEndian.Uint32(body[0:4]))

	// Test 5: New change (cas=ver+1000) with existing XATTR(pv and mv):
	meta, err = NewMetadataForTest(docKey, sourceClusterId, ver+1000, 1, []byte("0x0b0085b25e8d1416"), []byte("Cluster4"), []byte("0x0b0085b25e8d1416"),
		[]byte("{\"Cluster1\":\"0x0000f8da4d881416\"}"), []byte("{\"Cluster2\":\"0x1500f8da4d881416\",\"Cluster3\":\"0x2000f8da4d881416\"}"))
	assert.Nil(err)
	composer = base.NewXattrRawComposer(body)
	pos, _, err = ConstructXattrFromHlvForSetMeta(meta, 0, composer)
	assert.Nil(err)
...
func setMetaPruningWithNoNewChange(t *testing.T) {
	...
	p2 := fmt.Sprintf("\"Cluster2\":\"%s\"", base.Uint64ToHexLittleEndian(t2))
	p3 := fmt.Sprintf("\"Cluster3\":\"%s\"", base.Uint64ToHexLittleEndian(t3))
	p4 := fmt.Sprintf("\"Cluster4\":\"%s\"", base.Uint64ToHexLittleEndian(t4))
	p5 := fmt.Sprintf("\"Cluster5\":\"%s\"", base.Uint64ToHexLittleEndian(t5))
	...
	p2 = fmt.Sprintf("\"Cluster2\":\"%s\"", base.Uint64ToHexLittleEndian(t5))
	p3 = fmt.Sprintf("\"Cluster3\":\"%s\"", base.Uint64ToHexLittleEndian(t4))
	p4 = fmt.Sprintf("\"Cluster4\":\"%s\"", base.Uint64ToHexLittleEndian(t2))
	p5 = fmt.Sprintf("\"Cluster5\":\"%s\"", base.Uint64ToHexLittleEndian(t3))
	pv = fmt.Sprintf("{%s,%s,%s,%s}", p2, p3, p4, p5)
	meta, err = NewMetadataForTest(key, sourceClusterId, cas, 1, cvHex, []byte("Cluster1"), cvHex, []byte(pv), nil)
	assert.Nil(err)
	composer = base.NewXattrRawComposer(body)
	...
	p3 = fmt.Sprintf("\"Cluster3\":\"%s\"", base.Uint64ToHexLittleEndian(t3))
	p4 = fmt.Sprintf("\"Cluster4\":\"%s\"", base.Uint64ToHexLittleEndian(t4))
	p5 = fmt.Sprintf("\"Cluster5\":\"%s\"", base.Uint64ToHexLittleEndian(t2))
	pv = fmt.Sprintf("{\"Cluster2\":\"%s\",\"Cluster3\":\"%s\",\"Cluster4\":\"%s\",\"Cluster5\":\"%s\"}",
		base.Uint64ToHexLittleEndian(t5), base.Uint64ToHexLittleEndian(t3), base.Uint64ToHexLittleEndian(t4), base.Uint64ToHexLittleEndian(t2))
	meta, err = NewMetadataForTest(key, sourceClusterId, cas, 1, cvHex, []byte("Cluster1"), cvHex, []byte(pv), nil)
	assert.Nil(err)
	composer = base.NewXattrRawComposer(body)
	pos, _, err = ConstructXattrFromHlvForSetMeta(meta, 3*time.Second, composer)
...
func setMetaPruningWithNewChange(t *testing.T) {
	...
	p1 := fmt.Sprintf("\"Cluster1\":\"%s\"", base.Uint64ToHexLittleEndian(cv))
	p2 := fmt.Sprintf("\"Cluster2\":\"%s\"", base.Uint64ToHexLittleEndian(t2))
	p3 := fmt.Sprintf("\"Cluster3\":\"%s\"", base.Uint64ToHexLittleEndian(t3))
	p4 := fmt.Sprintf("\"Cluster4\":\"%s\"", base.Uint64ToHexLittleEndian(t4))
	p5 := fmt.Sprintf("\"Cluster5\":\"%s\"", base.Uint64ToHexLittleEndian(t5))
	...
	p1 = fmt.Sprintf("\"Cluster1\":\"%s\"", base.Uint64ToHexLittleEndian(t1))
	p2 = fmt.Sprintf("\"Cluster2\":\"%s\"", base.Uint64ToHexLittleEndian(t5))
	p3 = fmt.Sprintf("\"Cluster3\":\"%s\"", base.Uint64ToHexLittleEndian(t4))
	p4 = fmt.Sprintf("\"Cluster4\":\"%s\"", base.Uint64ToHexLittleEndian(t2))
	pv = fmt.Sprintf("{%s,%s,%s,%s}", p2, p3, p4, p5)
...
func setMetaPruningWithMvNoNewChange(t *testing.T) {
	...
	// Test 1. With 5 second pruning window, the whole pv survives
	v2 := fmt.Sprintf("\"Cluster2\":\"%s\"", base.Uint64ToHexLittleEndian(t2))
	v3 := fmt.Sprintf("\"Cluster3\":\"%s\"", base.Uint64ToHexLittleEndian(t3))
	v4 := fmt.Sprintf("\"Cluster4\":\"%s\"", base.Uint64ToHexLittleEndian(t4))
	v5 := fmt.Sprintf("\"Cluster5\":\"%s\"", base.Uint64ToHexLittleEndian(t5))
	...
	pv = fmt.Sprintf("{\"Cluster4\":\"%s\",\"Cluster5\":\"%s\",\"Cluster6\":\"%s\"}",
		base.Uint64ToHexLittleEndian(t5), base.Uint64ToHexLittleEndian(t4), base.Uint64ToHexLittleEndian(t6))
	meta, err = NewMetadataForTest(key, sourceClusterId, cas, 1, cvHex, []byte("Cluster1"), cvHex, []byte(pv), []byte(mv))
	assert.Nil(err)
	composer = base.NewXattrRawComposer(body)
	pos, _, err = ConstructXattrFromHlvForSetMeta(meta, 4*time.Second, composer)
	assert.Nil(err)
	assert.Contains(string(body[4:pos]), fmt.Sprintf("{\"cvCas\":\"%s\",\"src\":\"Cluster1\",\"ver\":\"%s\",", cvHex, cvHex))
	pvPruned := fmt.Sprintf("\"pv\":{\"Cluster5\":\"%s\"}", base.Uint64ToHexLittleEndian(t4))
...
func setMetaPruningWithMvNewChange(t *testing.T) {
	...
	v4 := fmt.Sprintf("\"Cluster4\":\"%s\"", base.Uint64ToHexLittleEndian(t4))
	v5 := fmt.Sprintf("\"Cluster5\":\"%s\"", base.Uint64ToHexLittleEndian(t5))
	v6 := fmt.Sprintf("\"Cluster6\":\"%s\"", base.Uint64ToHexLittleEndian(t6))
	mv := fmt.Sprintf("{%s,%s}", v2, v3)
...
func TestMergeMeta(t *testing.T) {
	...
	* 1. New at both source and target. Make sure we have MV but not PV.
	...
	assert.Contains(string(mv[:mvlen]), "\"SourceCluster\":\"0x2b4e85b25e8d1416\"")
	assert.Contains(string(mv[:mvlen]), "\"TargetCluster\":\"0x1b2785b25e8d1416\"")
	...
	* oldXattr = "_vv\x00{\"src\":\"Cluster4\",\"ver\":\"0x0b0085b25e8d1416\",\"pv\":{\"Cluster1\":\"0x0000f8da4d881416\",\"Cluster2\":\"0x1500f8da4d881416\",\"Cluster3\":\"0x2000f8da4d881416\"}}\x00"
	 */
	...
	assert.Contains(string(mv[:mvlen]), "\"SourceCluster\":\"0x2b4e85b25e8d1416\"")
	assert.Contains(string(mv[:mvlen]), "\"TargetCluster\":\"0x1b2785b25e8d1416\"")
	assert.Contains(string(pv[:pvlen]), "\"Cluster1\":\"0x0000f8da4d881416\"")
	assert.Contains(string(pv[:pvlen]), "\"Cluster2\":\"0x1500f8da4d881416\"")
	assert.Contains(string(pv[:pvlen]), "\"Cluster3\":\"0x2000f8da4d881416\"")
	assert.Contains(string(pv[:pvlen]), "\"Cluster4\":\"0x0b0085b25e8d1416\"")
	...
	* Target cluster contains changes coming from cluster5: "_vv\x00{\"src\":\"Cluster5\"ver\":\"0x0b0085b25e8d1416\",\"pv\":{\"Cluster1\":\"0x0000f8da4d881416\",\"Cluster2\":\"0x1500f8da4d881416\",\"Cluster3\":\"0x2000f8da4d881416\"}}\x00"
	 */
	...
	assert.Nil(err)
	targetMeta, err = NewMetadataForTest(key, targetClusterId, cv, 1, []byte("0x0b0085b25e8d1416"), []byte("Cluster5"), []byte("0x0b0085b25e8d1416"), []byte("{\"Cluster1\":\"0x0000f8da4d881416\",\"Cluster2\":\"0x1500f8da4d881416\",\"Cluster3\":\"0x2000f8da4d881416\"}"), nil)
	assert.Nil(err)
	mergedMeta, err = sourceMeta.Merge(targetMeta)
	assert.Nil(err)
	mvlen, _ = VersionMapToBytes(mergedMeta.GetHLV().GetMV(), mv, 0, nil)
	pvlen, _ = VersionMapToBytes(mergedMeta.GetHLV().GetPV(), pv, 0, nil)
	assert.Contains(string(mv[:mvlen]), "\"Cluster4\":\"0x0b0085b25e8d1416\"")
	assert.Contains(string(mv[:mvlen]), "\"Cluster5\":\"0x0b0085b25e8d1416\"")
	assert.Contains(string(pv[:pvlen]), "\"Cluster1\":\"0x0000f8da4d881416\"")
	assert.Contains(string(pv[:pvlen]), "\"Cluster2\":\"0x1500f8da4d881416\"")
	assert.Contains(string(pv[:pvlen]), "\"Cluster3\":\"0x2000f8da4d881416\"")
	...
	* 4. Source and target both updated. Both have pv, one has mv
	 */
	cv, _ = base.HexLittleEndianToUint64([]byte("0x0b0085b25e8d1416"))
	sourceMeta, err = NewMetadataForTest(key, sourceClusterId, cv+20000, 1, []byte("0x0b0085b25e8d1416"), []byte("Cluster4"), []byte("0x0b0085b25e8d1416"), []byte("{\"Cluster1\":\"0x0000f8da4d881416\"}"), []byte("{\"Cluster3\":\"0x2000f8da4d881416\",\"Cluster2\":\"0x1500f8da4d881416\"}"))
	assert.Nil(err)
	targetMeta, err = NewMetadataForTest(key, targetClusterId, cv+10000, 1, []byte("0x0b0085b25e8d1416"), []byte("Cluster5"), []byte("0x0b0085b25e8d1416"), []byte("{\"Cluster1\":\"0x0000f8da4d881416\",\"Cluster2\":\"0x1500f8da4d881416\"}"), nil)
	assert.Nil(err)
	mergedMeta, err = sourceMeta.Merge(targetMeta)
	assert.Nil(err)
	mvlen, _ = VersionMapToBytes(mergedMeta.GetHLV().GetMV(), mv, 0, nil)
	...
	* 5. Source is a merged doc. Target is an update with pv
	...
	sourceMeta, err = NewMetadataForTest(key, sourceClusterId, cv, 1, []byte("0x0b0085b25e8d1416"), []byte("Cluster4"), []byte("0x0b0085b25e8d1416"), nil, []byte("{\"Cluster1\":\""+string(c1)+"\",\"Cluster3\":\"0x2000f8da4d881416\"}"))
	assert.Nil(err)
	targetMeta, err = NewMetadataForTest(key, targetClusterId, cv+10000, 1, []byte("0x0b0085b25e8d1416"), []byte("Cluster5"), []byte("0x0b0085b25e8d1416"), []byte("{\"Cluster1\":\""+string(c2)+"\",\"Cluster2\":\"0x1500f8da4d881416\"}"), nil)
	assert.Nil(err)
	mergedMeta, err = sourceMeta.Merge(targetMeta)
	assert.Nil(err)
	mvlen, _ = VersionMapToBytes(mergedMeta.GetHLV().GetMV(), mv, 0, nil)
	pvlen, _ = VersionMapToBytes(mergedMeta.GetHLV().GetPV(), pv, 0, nil)
	assert.Contains(string(mv[:mvlen]), "\"TargetCluster\":\"0x1b2785b25e8d1416\"")
	assert.Contains(string(mv[:mvlen]), "\"Cluster1\":\"0x0000f8da4d881416\"")
	assert.Contains(string(mv[:mvlen]), "\"Cluster3\":\"0x2000f8da4d881416\"")
	assert.Contains(string(pv[:pvlen]), "\"Cluster2\":\"0x1500f8da4d881416\"")
	assert.Contains(string(pv[:pvlen]), "\"Cluster5\":\"0x0b0085b25e8d1416\"")

	/*
	* 6. Target is a merged doc. Source is an update with PV and MV
	 */
	sourceMeta, err = NewMetadataForTest(key, sourceClusterId, cv+10000, 1, []byte("0x0b0085b25e8d1416"), []byte("Cluster5"), []byte("0x0b0085b25e8d1416"), []byte("{\"Cluster1\":\""+string(c2)+"\",\"Cluster2\":\"0x1500f8da4d881416\"}"), nil)
	assert.Nil(err)
	targetMeta, err = NewMetadataForTest(key, targetClusterId, cv, 1, []byte("0x0b0085b25e8d1416"), []byte("Cluster4"), []byte("0x0b0085b25e8d1416"), nil, []byte("{\"Cluster1\":\""+string(c1)+"\",\"Cluster3\":\"0x2000f8da4d881416\"}"))
	assert.Nil(err)
	mergedMeta, err = sourceMeta.Merge(targetMeta)
	assert.Nil(err)
	mvlen, _ = VersionMapToBytes(mergedMeta.GetHLV().GetMV(), mv, 0, nil)
	pvlen, _ = VersionMapToBytes(mergedMeta.GetHLV().GetPV(), pv, 0, nil)
	assert.Contains(string(mv[:mvlen]), "\"SourceCluster\":\"0x1b2785b25e8d1416\"")
	assert.Contains(string(mv[:mvlen]), "\"Cluster1\":\"0x0000f8da4d881416\"")
	assert.Contains(string(mv[:mvlen]), "\"Cluster3\":\"0x2000f8da4d881416\"")
	assert.Contains(string(pv[:pvlen]), "\"Cluster2\":\"0x1500f8da4d881416\"")
	assert.Contains(string(pv[:pvlen]), "\"Cluster5\":\"0x0b0085b25e8d1416\"")
	...
	* 7. Both are merged docs.
	 */
	sourceMeta, err = NewMetadataForTest(key, sourceClusterId, cv, 1, []byte("0x0b0085b25e8d1416"), []byte("Cluster5"), []byte("0x0b0085b25e8d1416"), nil, []byte("{\"Cluster1\":\""+string(c2)+"\",\"Cluster2\":\"0x1500f8da4d881416\"}"))
	assert.Nil(err)
	targetMeta, err = NewMetadataForTest(key, targetClusterId, cv, 1, []byte("0x0b0085b25e8d1416"), []byte("Cluster4"), []byte("0x0b0085b25e8d1416"), nil, []byte("{\"Cluster1\":\""+string(c1)+"\",\"Cluster3\":\"0x2000f8da4d881416\"}"))
	assert.Nil(err)
	mergedMeta, err = sourceMeta.Merge(targetMeta)
	...
	pvlen, _ = VersionMapToBytes(mergedMeta.GetHLV().GetPV(), pv, 0, nil)
	assert.Contains(string(mv[:mvlen]), "\"Cluster1\":\"0x0000f8da4d881416\"")
	assert.Contains(string(mv[:mvlen]), "\"Cluster2\":\"0x1500f8da4d881416\"")
	assert.Contains(string(mv[:mvlen]), "\"Cluster3\":\"0x2000f8da4d881416\"")
	assert.Equal(0, pvlen)
	...
	* 8. Source is a new change. Target has a history
	...
	assert.Nil(err)
	targetMeta, err = NewMetadataForTest(key, targetClusterId, cv+1000, 1, []byte("0x0b0085b25e8d1416"), []byte("Cluster4"), []byte("0x0b0085b25e8d1416"), []byte("{\"Cluster1\":\""+string(c1)+"\",\"Cluster3\":\"0x2000f8da4d881416\"}"), nil)
	assert.Nil(err)
	mergedMeta, err = sourceMeta.Merge(targetMeta)
	assert.Nil(err)
	mvlen, _ = VersionMapToBytes(mergedMeta.GetHLV().GetMV(), mv, 0, nil)
	...
	assert.Contains(string(pv[:pvlen]), "\"Cluster3\":\"0x2000f8da4d881416\"")
	assert.Contains(string(pv[:pvlen]), "\"Cluster4\":\"0x0b0085b25e8d1416\"")
	assert.Contains(string(mv[:mvlen]), "\"SourceCluster\":\"0xdb0785b25e8d1416\"")
	assert.Contains(string(mv[:mvlen]), "\"TargetCluster\":\"0xf30385b25e8d1416\"")
}
...
func TestUpdateMetaForSetBack(t *testing.T) {
	...
	* 1. Target has a PV, no MV, and no new update at TargetCluster. srv/ver (Cluster4) is added to pv
	...
	assert.Nil(err)
	targetMeta, err := NewMetadataForTest(key, targetClusterId, cv, 1, cvHex, []byte("Cluster4"), cvHex, []byte("{\"Cluster1\":\"0x0000f8da4d881416\",\"Cluster3\":\"0x2000f8da4d881416\"}"), nil)
	assert.Nil(err)
	pv, mv, err := targetMeta.UpdateMetaForSetBack()
	assert.Nil(err)
	assert.Nil(mv)
	assert.Contains(string(pv), "\"Cluster1\":\"0x0000f8da4d881416\"")
	assert.Contains(string(pv), "\"Cluster3\":\"0x2000f8da4d881416\"")
	assert.Contains(string(pv), "\"Cluster4\":\"0x0b0085b25e8d1416\"")
	...
	* 2. Target has a PV, no MV, and new update. ver/src and cas/source are added to pv
	...
	cv, err = base.HexLittleEndianToUint64(cvHex)
	assert.Nil(err)
	targetMeta, err = NewMetadataForTest(key, targetClusterId, cv+1000, 1, cvHex, []byte("Cluster4"), cvHex, []byte("{\"Cluster1\":\"0x0000f8da4d881416\",\"Cluster3\":\"0x2000f8da4d881416\"}"), nil)
	assert.Nil(err)
	pv, mv, err = targetMeta.UpdateMetaForSetBack()
	assert.Nil(err)
	assert.Nil(mv)
	...
	assert.Contains(string(pv), "\"Cluster3\":\"0x2000f8da4d881416\"")
	assert.Contains(string(pv), "\"Cluster4\":\"0x0b0085b25e8d1416\"")
	assert.Contains(string(pv), "\"TargetCluster\":\"0xf30385b25e8d1416\"")

	/*
	* 3. Target is a merged docs with no new change.
	 */
	targetMeta, err = NewMetadataForTest(key, targetClusterId, cv, 1, []byte("0x0b0085b25e8d1416"), []byte("Cluster4"), []byte("0x0b0085b25e8d1416"), nil, []byte("{\"Cluster1\":\"0x0000f8da4d881416\",\"Cluster3\":\"0x2000f8da4d881416\"}"))
	assert.Nil(err)

	pv, mv, err = targetMeta.UpdateMetaForSetBack()
	assert.Nil(err)
	assert.Nil(pv)
	assert.Contains(string(mv), "\"Cluster1\":\"0x0000f8da4d881416\"")
	assert.Contains(string(mv), "\"Cluster3\":\"0x2000f8da4d881416\"")

	/*
	* 4. Target is a merged docs with new change.
	 */
	targetMeta, err = NewMetadataForTest(key, targetClusterId, cv+1000, 1, []byte("0x0b0085b25e8d1416"), []byte("Cluster4"), []byte("0x0b0085b25e8d1416"), []byte("{\"Cluster2\":\"0x1500f8da4d881416\"}"), []byte("{\"Cluster1\":\"0x0000f8da4d881416\",\"Cluster3\":\"0x2000f8da4d881416\"}"))
	assert.Nil(err)

	pv, mv, err = targetMeta.UpdateMetaForSetBack()
	assert.Nil(mv)
	assert.Contains(string(pv), "\"Cluster1\":\"0x0000f8da4d881416\"")
	assert.Contains(string(pv), "\"Cluster2\":\"0x1500f8da4d881416\"")
	assert.Contains(string(pv), "\"Cluster3\":\"0x2000f8da4d881416\"")
	assert.Contains(string(pv), "\"TargetCluster\":\"0xf30385b25e8d1416\"")
}
