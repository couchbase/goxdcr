...
// ParseHlvFields parses the various feilds of HLV from the input _vv xattr.
// Note that for pure Couchbase Server deployments, the invariant cvVer <= cvCas <= document CAS is true.
// However if other actors like Couchbase Mobile are in the mix, the clocks used to generate these various fields
// might differ from actor to actor. Therefore the above mentioned invariant might not hold true and they
// should not be compared against each other. This should be functionally alright as long as each clock used by the
// actors are monotonically increasing (eg: HLC), since conflict detection algorithm using HLV only ever compares
// versions generated by the same actors.
func ParseHlvFields(xattr []byte) (cvCas uint64, src hlv.DocumentSourceId, cvVer uint64, pvMap, mvMap hlv.VersionsMap, err error) {
...
func getHlvFromMCResponse(lookupResp *base.WrappedMCResponse) (cas, cvCas uint64, cvSrc hlv.DocumentSourceId, cvVer uint64,
	...
	if xattr != nil && !base.Equals(xattr, base.EmptyJsonObject) {
		cvCas, cvSrc, cvVer, pvMap, mvMap, err1 = ParseHlvFields(xattr)
		if err1 != nil {
...
func getHlvFromMCRequest(wrappedReq *base.WrappedMCRequest, uncompressFunc base.UncompressFunc) (cas, cvCas uint64, cvSrc hlv.DocumentSourceId, cvVer uint64, pvMap, mvMap hlv.VersionsMap, importCas uint64, pRev uint64, err error) {
	...
	cvCas, cvSrc, cvVer, pvMap, mvMap, err1 = ParseHlvFields(xattrHlv)
	if err1 != nil {
