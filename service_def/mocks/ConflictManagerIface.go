// Code generated by mockery v1.0.0. DO NOT EDIT.

package mocks

import base "github.com/couchbase/goxdcr/base"
import gomemcached "github.com/couchbase/gomemcached"
import mock "github.com/stretchr/testify/mock"

// ConflictManagerIface is an autogenerated mock type for the ConflictManagerIface type
type ConflictManagerIface struct {
	mock.Mock
}

// ResolveConflict provides a mock function with given fields: source, target, sourceId, targetId
func (_m *ConflictManagerIface) ResolveConflict(source *base.WrappedMCRequest, target *gomemcached.MCResponse, sourceId []byte, targetId []byte) error {
	ret := _m.Called(source, target, sourceId, targetId)

	var r0 error
	if rf, ok := ret.Get(0).(func(*base.WrappedMCRequest, *gomemcached.MCResponse, []byte, []byte) error); ok {
		r0 = rf(source, target, sourceId, targetId)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}
