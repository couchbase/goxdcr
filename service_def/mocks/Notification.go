// Code generated by mockery v2.5.1. DO NOT EDIT.

package mocks

import (
	service_def "github.com/couchbase/goxdcr/service_def"
	mock "github.com/stretchr/testify/mock"
)

// Notification is an autogenerated mock type for the Notification type
type Notification struct {
	mock.Mock
}

// CloneRO provides a mock function with given fields:
func (_m *Notification) CloneRO() service_def.Notification {
	ret := _m.Called()

	var r0 service_def.Notification
	if rf, ok := ret.Get(0).(func() service_def.Notification); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(service_def.Notification)
		}
	}

	return r0
}

// GetKvVbMapRO provides a mock function with given fields:
func (_m *Notification) GetKvVbMapRO() (map[string][]uint16, error) {
	ret := _m.Called()

	var r0 map[string][]uint16
	if rf, ok := ret.Get(0).(func() map[string][]uint16); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(map[string][]uint16)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func() error); ok {
		r1 = rf()
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// GetNumberOfSourceNodes provides a mock function with given fields:
func (_m *Notification) GetNumberOfSourceNodes() (int, error) {
	ret := _m.Called()

	var r0 int
	if rf, ok := ret.Get(0).(func() int); ok {
		r0 = rf()
	} else {
		r0 = ret.Get(0).(int)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func() error); ok {
		r1 = rf()
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// GetSourceVBMapRO provides a mock function with given fields:
func (_m *Notification) GetSourceVBMapRO() (map[string][]uint16, error) {
	ret := _m.Called()

	var r0 map[string][]uint16
	if rf, ok := ret.Get(0).(func() map[string][]uint16); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(map[string][]uint16)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func() error); ok {
		r1 = rf()
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// IsSourceNotification provides a mock function with given fields:
func (_m *Notification) IsSourceNotification() bool {
	ret := _m.Called()

	var r0 bool
	if rf, ok := ret.Get(0).(func() bool); ok {
		r0 = rf()
	} else {
		r0 = ret.Get(0).(bool)
	}

	return r0
}
