...
func (rctx *refreshContext) checkAndUpdateAgentReference() error {
	...
	if !rctx.refOrig.IsSame(rctx.refCache) || rctx.nodeListUpdated || rctx.addressPrefUpdate {
...
func (rctx *refreshContext) verifyNodeAndGetList(connStr string, updateSecuritySettings bool) ([]interface{}, error) {
	...
	backgroundRpc := func(opaque uint64) {
		...
		if updateSecuritySettings && rctx.refCache.IsEncryptionEnabled() {
			...
			httpAuthMech, defaultPoolInfo, bgErr = rctx.agent.utils.GetSecuritySettingsAndDefaultPoolInfo(rctx.hostName, rctx.httpsHostName, username, password, certificate, clientCertificate, clientKey, rctx.refCache.IsHalfEncryption(), rctx.agent.logger)
			if bgErr != nil {
