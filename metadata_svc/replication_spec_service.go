...
func (service *ReplicationSpecService) ValidateNewReplicationSpec(sourceBucket, targetCluster, targetBucket string, settings metadata.ReplicationSettingsMap, performRemoteValidation bool) (string, string, *metadata.RemoteClusterReference, base.ErrorMap, error, service_def.UIWarnings, *metadata.CollectionsManifestPair) {
		...
		go func() {
			...
			if performRemoteValidation {
				...
				if !targetClusterRef.IsCNG() {
					...
					hasSharedExternalHostname, err := service.utils.TargetHasSharedExternalHostnameAndMgmtPort(targetBucketInfo, useExternal)
					if err != nil {
	...
	if performRemoteValidation && sourceBucketNumberOfVbs != targetBucketNumberOfVbs && !base.IsClusterCompatible(sourceCompat, base.VersionForVariableVBSupport) {
		errMsg := fmt.Sprintf("The number of vbuckets in source cluster, %v, and target cluster, %v, does not match. This configuration is not supported until this cluster is fully upgraded to %s or above.", sourceBucketNumberOfVbs, targetBucketNumberOfVbs, base.VersionForVariableVBSupport.String())
		service.logger.Error(errMsg)
...
func (service *ReplicationSpecService) validateReplicationSettingsLocal(errorMap base.ErrorMap, sourceBucket, targetCluster, targetBucket string, settings metadata.ReplicationSettingsMap, newSettings bool, warnings service_def.UIWarnings) error {
	...
	if forwardLocalOnly, ok := settings[metadata.ForwardLocalOnlyKey].(bool); ok && forwardLocalOnly {
		...
		// As part of the CNG-P1 guardrails, remote-cluster cannot be of CNG RemoteType
		if errTargetClusterRef != nil {
			return fmt.Errorf("failed to verify remote-cluster type - %v", errTargetClusterRef)
		}
		if targetClusterRef.RemoteType == metadata.RemoteTypeCng {
			errorMap[base.ForwardLocalOnlyKey] = base.ErrorForwardLocalOnlyCNGUnsupported
			return nil
		}
	}
