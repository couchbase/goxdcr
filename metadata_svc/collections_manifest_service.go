...
func (a *CollectionsManifestAgent) refreshTargetCustom(force, lock bool, waitTime time.Duration, maxRetry int) (oldManifest, newManifest *metadata.CollectionsManifest, err error) {
	...
	getRetry := func() error {
		...
		getManifestReqOpts := base.NewGetManifestOpts(clusterUuid, bucketName, force, a.tempAgent)
		manifest, err = a.remoteClusterSvc.GetManifestByUuid(getManifestReqOpts)
