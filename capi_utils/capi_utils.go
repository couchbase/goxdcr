...
func ConstructServerCouchApiBaseMap(targetBucket *couchbase.Bucket, remoteClusterRef *metadata.RemoteClusterReference) (map[string]string, error) {
	...
	err, _ := utils.QueryRestApiWithAuth(remoteClusterRef.HostName, targetBucket.URI, false, remoteClusterRef.UserName, remoteClusterRef.Password, []byte{}, base.MethodGet, "", nil, 0, &out, nil, false, logger_capi_utils)
	if err != nil {
