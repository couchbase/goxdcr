...
// contructTargetKvVbMap returns a map of remote KV node -> vbuckets it owns.
// It uses cachedTargetKvVbMap from BucketTopologyService when available to avoid a redundant terse bucket info RPC.
func (xdcrf *XDCRFactory) contructTargetKvVbMap(topic string, spec *metadata.ReplicationSpecification, targetBucketInfo map[string]interface{}, targetClusterRef *metadata.RemoteClusterReference, useExternal bool, cachedTargetKvVbMap base.KvVBMapType) (map[string][]uint16, error) {
	if len(cachedTargetKvVbMap) > 0 {
		xdcrf.logger.Infof("using cached target kv_vb_map from bucket topology service during pipeline creation, topic=%s", topic)
		return map[string][]uint16(cachedTargetKvVbMap), nil
	}
...
func (xdcrf *XDCRFactory) constructOutgoingNozzles(topic string, spec *metadata.ReplicationSpecification, kv_vb_map map[string][]uint16,
	...
	pipelineType common.PipelineType, kvsVbMap base.KvVBMapType, cachedTargetKvVbMap base.KvVBMapType) (outNozzles map[string]common.Nozzle,
	...
	kvVBMap, err = xdcrf.contructTargetKvVbMap(topic, spec, targetBucketInfo, targetClusterRef, useExternal, cachedTargetKvVbMap)
	if err != nil {
		return
	}
	...
	for kvaddr, kvVBList := range kvVBMap {
		...
		for i := 0; i < numOfOutNozzles; i++ {
			...
			if nozzleType == base.Capi {
				...
				outNozzle = xdcrf.constructXMEMNozzle(topic, spec.SourceBucketUUID, spec.TargetClusterUUID, kvaddr, spec.SourceBucketName, spec.TargetBucketName, spec.TargetBucketUUID,
					targetUserName, targetPassword, i, connSize, sourceCRMode, logger_ctx, vbList, eventsProducer, sourceClusterUUID, sourceHostname, pipelineType)
			}
...
func (xdcrf *XDCRFactory) constructRouter(id string, spec *metadata.ReplicationSpecification, downStreamParts map[string]common.Part, vbNozzleMap map[uint16]string, sourceCRMode base.ConflictResolutionMode, logger_ctx *log.LoggerContext, srcNozzleObjRecycler utilities.RecycleObjFunc, migrationUIMsgRaiser func(string), srcNozzleVBs []uint16, allSrcVBs []uint16) (*parts.Router, error) {
	...
	if err != nil {
		...
		xdcrf.logger.Infof("Constructed router %v with priority=%s responsible VBs %v", routerId, spec.Settings.GetPriority().String(), router.GetPart().ResponsibleVBs())
	}
	return router, err
}
...
func (xdcrf *XDCRFactory) constructXMEMNozzle(topic string,
	...
	nozzle := parts.NewXmemNozzle(xmemNozzle_Id, xdcrf.remote_cluster_svc, sourceBucketUuid, targetClusterUuid, topic, topic, connPoolSize, kvaddr, sourceBucketName, targetBucketName,
		targetBucketUuid, username, password, sourceCRMode, logger_ctx, xdcrf.utils, vbList, eventsProducer, sourceClusterUUID, sourceHostname, pipelineType)
	return nozzle
}
...
func (xdcrf *XDCRFactory) registerServices(pipeline common.Pipeline, logger_ctx *log.LoggerContext, kv_vb_map map[string][]uint16, targetUserName, targetPassword, targetBucketName string, target_kv_vb_map map[string][]uint16, targetClusterRef *metadata.RemoteClusterReference, nozzleType base.XDCROutgoingNozzleType, mainPipeline *common.Pipeline, crMode base.ConflictResolutionMode, variableVBMode bool) error {
	...
	// register conflict logger to the pipeline, if it exists.
	if conflictLogger != nil {
		err = ctx.RegisterService(base.CONFLICT_LOGGER, conflictLogger)
		if err != nil {
			return err
		}
	}
	...
	// and conflict logger is registered.
	...
	if mainPipeline != nil {
		...
		top_detect_svc = pipeline_svc.NewTopologyChangeDetectorSvc(xdcrf.xdcr_topology_svc, xdcrf.remote_cluster_svc, xdcrf.repl_spec_svc, logger_ctx, xdcrf.utils, xdcrf.bucketTopologySvc, variableVBMode, nozzleType == base.CNG)
	}
