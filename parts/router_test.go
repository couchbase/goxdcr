...
func TestRouterCasDriftCheck(t *testing.T) {
	...
	oneSec := time.Duration(1) * time.Second
	driftThreshold := time.Duration(router.casDriftThreshold) * time.Second
	acceptableDriftTime := time.Now().Add(driftThreshold).Add(-oneSec).UnixNano()
...
func TestMigrationFilterAfterStrippingTxnXattr(t *testing.T) {
	...
	router, err := NewRouter(routerId, spec, downStreamParts, routingMap, crMode, loggerCtx, utilities.NewUtilities(), throughputThrottlerSvc, needToThrottle, expDelMode, collectionsManifestSvc, recycler, nil, nonCollectionsCap, nil, connectivityStatus, nil)
	a.Nil(err)
	...
	a.Nil(err)
	body, _ = composer.FinishAndAppendDocValue([]byte(val))
	...
	router.filter, err = filter.NewFilterWithSharedDP("", "", utilities.NewUtilities(), base.NewFakeDataPool(), 0)
	a.Nil(err)
	...
	replicate, err, errDesc, dpFails := router.filter.FilterUprEvent(uprEvent)
	a.True(replicate)
	a.Nil(err)
