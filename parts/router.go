...
func (router *Router) ComposeMCRequest(wrappedEvent *base.WrappedUprEvent) (*base.WrappedMCRequest, error) {
	...
	if event.Opcode == mc.UPR_MUTATION || event.Opcode == mc.UPR_DELETION ||
		...
		if router.sourceCRMode == base.CRMode_LWW {
			...
			// For mobile, we also skip target conflict resolution. This will be done in xmem based on
			// whether the mutation is being sent to target with mobile enabled. Setting it here can have
			// race condition since the setting cab change in the middle of the replication.
