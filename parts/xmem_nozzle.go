...
func (xmem *XmemNozzle) detectConflictWithXattr(source *mc.MCRequest, lookupResp *base.SubdocLookupResponse) (ConflictResult, error) {
	 ...
	 */
	if sourceMeta.GetCas() == targetMeta.GetCas() && bytes.Equal(sourceMeta.Updater(), targetMeta.Updater()) {
	...
	if targetMeta.IsMergedDoc() {
		...
		sourceContainTarget, err = sourceMeta.ContainsVersion(targetMeta.Updater(), targetMeta.GetCas())
	}
	if err != nil {
		return Conflict, err
	}
	if sourceMeta.IsMergedDoc() {
		...
		targetContainSource, err = targetMeta.ContainsVersion(sourceMeta.Updater(), sourceMeta.GetCas())
	}
