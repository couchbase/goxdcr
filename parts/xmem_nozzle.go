...
func (buf *requestBuffer) enSlot(mcReq *base.WrappedMCRequest) (uint16, int, []byte, error) {
	...
	if dpErr != nil {
		if buf.devReplOpts.StrictDataPoolUse {
			return 0, 0, nil, dpErr
		}
			...
			case Send:
				...
				if err != nil {
					if err == base.ErrorSubdocMaxPathLimitBreached {
						...
						xmem.RaiseEvent(common.NewEvent(common.SubdocCmdSkippedDueToLimits, nil, xmem, []interface{}{item.GetSourceVB(), item.GetTargetVB(), item.Seqno}, additionalInfo))
...
func (xmem *XmemNozzle) ComposeRequestForSubdocGet(specs base.SubdocLookupPathSpecs, wrappedReq *base.WrappedMCRequest, opaque uint32, cas uint64) *base.WrappedMCRequest {
	...
	if dpErr != nil {
		...
		if xmem.devReplOpts.StrictDataPoolUse {
			...
			panic(fmt.Sprintf("%s ComposeRequestForSubdocGet: DataPool GetByteSlice failed even with StrictDataPoolUse option on, err=%v", xmem.Id(), dpErr))
		}
	} else {
...
func (xmem *XmemNozzle) initialize(settings metadata.ReplicationSettingsMap) error {
	...
	xmem.setRequestBuffer(newReqBuffer(uint16(xmem.config.maxCount*2),
		uint16(float64(xmem.config.maxCount)*0.2),
		xmem.receive_token_ch, xmem.Logger(),
		xmem.dataPool,
		xmem.devReplOpts))
			...
			if response.Status != mc.SUCCESS && !base.IsIgnorableMCResponse(response, wrappedReq.IsCasLockingRequest()) {
				if base.IsMutationLockedError(response.Status) {
					...
					if base.IsTopologyChangeMCError(response.Status) {
						...
						upstreamReporter, exists := xmem.upstreamErrReporterMap[wrappedReq.GetSourceVB()]
						if !exists {
