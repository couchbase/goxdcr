...
func testTunables() base.RemoteMemcachedTunables {
	tunables := base.NewRemoteMemcachedTunables()
	tunables.MaxConnsPerServer = testMaxConnectionsPerServer
	return tunables
}
...
func setupTestRemoteMemcachedComponent(provider *ClusterBucketStatsProvider, utils *utilsMock.UtilsIface, logger *log.CommonLogger) {
	...
	provider.remoteMemcachedComponent = Component.NewRemoteMemcachedComponent(logger, provider.finCh, utils, testBucketName, "test", testTunables())
...
func setupClusterBucketStatsProvider() (*ClusterBucketStatsProvider, *utilsMock.UtilsIface, *service_defMock.BucketTopologySvc, *service_defMock.RemoteClusterSvc) {
	...
	provider := NewClusterBucketStatsProvider(testBucketName, testClusterUuid, remoteClusterSvc, utils, bucketTopologySvc, testTunables(), logger, createTestGetTargetKvVbMapFunc())
...
func TestClusterBucketStatsProvider_Init_TargetBucketTopologyNotReady(t *testing.T) {
	...
	provider := NewClusterBucketStatsProvider(testBucketName, testClusterUuid, remoteClusterSvc, utils, bucketTopologySvc, testTunables(), logger, getTargetKvVbMapFunc)
...
func TestClusterBucketStatsProvider_GetFailoverLog_Success(t *testing.T) {
	...
	utils.On("GetFailoverLog", mcClient, mock.Anything).Return(failoverLog.FailoverLogMap, nil, nil)
...
func TestClusterBucketStatsProvider_GetFailoverLog_PartialFailure(t *testing.T) {
	...
	utils.On("GetFailoverLog", mcClient, mock.Anything).Return(failoverLogServer1.FailoverLogMap, nil, nil)

	// Second server fails
	utils.On("GetFailoverLog", mcClient2, mock.Anything).Return(nil, nil, errors.New("failed to get failover log"))
...
func TestClusterBucketStatsProvider_GetFailoverLog_NetworkError(t *testing.T) {
	...
	utils.On("GetFailoverLog", mock.Anything, mock.Anything).Return(nil, nil, networkErr)
...
func TestClusterBucketStatsProvider_GetFailoverLog_ParseErrors(t *testing.T) {
	...
	utils.On("GetFailoverLog", mcClient, mock.Anything).Return(failoverLogMap, vbErrorMap, nil)
	utils.On("IsSeriousNetError", mock.Anything).Return(false)
...
func TestClusterBucketStatsProvider_GetFailoverLog_MixedSuccessAndParseErrors(t *testing.T) {
	...
	utils.On("GetFailoverLog", mcClient, mock.Anything).Return(failoverLogServer1, vbErrorMapServer1, nil)
	utils.On("GetFailoverLog", mcClient2, mock.Anything).Return(failoverLogServer2, nil, nil)
	utils.On("IsSeriousNetError", mock.Anything).Return(false)
...
func TestClusterBucketStatsProvider_ConcurrentOperationsAndClose_NoDeadlock(t *testing.T) {
	...
	mockUtils.On("GetFailoverLog", mock.Anything, mock.Anything).Return(make(base.FailoverLogMapType), nil, nil, nil)
