...
func (u *Utilities) GetMemcachedSSLPortMap(connStr, username, password string, authMech base.HttpAuthMech, certificate []byte, sanInCertificate bool,
	...
	bucketInfoReq := &GetBucketInfoReq{
		FromCNG:           false, // GetMemcachedSSLPortMap is called in the context of non-CNG ref
		HostAddr:          connStr,
		BucketName:        bucket,
		Username:          username,
		Password:          password,
	...
	nonTerseInfo, err := u.GetBucketInfo(logger, bucketInfoReq)
	if err != nil {
		return nil, err
	}
...
func (u *Utilities) bucketValidationInfoInternal(logger *log.CommonLogger, req *GetBucketInfoReq, useExternal bool) (bucketInfo map[string]interface{}, bucketType string, bucketUUID string, bucketConflictResolutionType string,
	bucketEvictionPolicy string, bucketKVVBMap map[string][]uint16, err error) {
	...
	bucketValidationInfoOp := func() error {
		...
		var shouldUseTerseInfo bool

		if !IsBucketInfoFromCng(bucketInfo) {
			shouldUseTerseInfo, err = u.ShouldUseTerseBucketInfo(bucketInfo, req.HostAddr, req.BucketName, useExternal, req.HTTPAuthMech == base.HttpAuthMechHttps)
			if err != nil {
				err = fmt.Errorf("error checking for the need of terse info for bucket %v: %w", req.BucketName, err)
				return err
			}
		}
		...
		if shouldUseTerseInfo {
			terseBucketInfo, err := u.GetTerseBucketInfo(req.HostAddr, req.BucketName, req.Username, req.Password, req.HTTPAuthMech, req.Certificate, req.SanInCertificate, req.ClientCertificate, req.ClientKey, logger)
			if err != nil {
				err = fmt.Errorf("error getting terse info for bucket %v: %w", req.BucketName, err)
				return err
			}
		...
		bucketKVVBMapPtr, err = u.GetServerVBucketsMap(req.HostAddr, req.BucketName, bucketInfoForKvVBMap, nil, nil)
		if err != nil {
			err = fmt.Errorf("error getServerVBucketsMap on bucket %v. err=%v", req.BucketName, err)
			return err
		}
...
func (u *Utilities) GetFailoverLog(conn mcc.ClientIface, dataTransferCtx *Context) (base.FailoverLogMapType, map[uint16]error, error) {
