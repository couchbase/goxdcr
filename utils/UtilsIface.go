...
type UtilsIface interface {
	...
	TargetHasSharedExternalHostnameAndMgmtPort(bucketInfo map[string]interface{}, useExternal bool) (bool, error)
	...
	GetFailoverLog(conn mcc.ClientIface, dataTransferCtx *Context) (base.FailoverLogMapType, map[uint16]error, error)
	GetVBucketStats(requestOpts *base.VBucketStatsRequest, conn mcc.ClientIface, dataTransferCtx *Context) (base.VBucketStatsMap, error)
	...
	GetNodeExtListFromInfoMap(infoMap map[string]interface{}, logger *log.CommonLogger) ([]interface{}, error)
