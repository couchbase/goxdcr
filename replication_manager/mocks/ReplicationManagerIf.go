...
func (_m *ReplicationManagerIf) createAndPersistReplicationSpec(justValidate bool, sourceBucket string, targetCluster string, targetBucket string, settings map[string]interface{}) (*metadata.ReplicationSpecification, map[string]error, error, []string) {
	...
	var r3 []string
	if rf, ok := ret.Get(3).(func(bool, string, string, string, map[string]interface{}) []string); ok {
		r3 = rf(justValidate, sourceBucket, targetCluster, targetBucket, settings)
	} else {
		if ret.Get(3) != nil {
			r3 = ret.Get(3).([]string)
		}
...
// initReplications provides a mock function with given fields:
func (_m *ReplicationManagerIf) initReplications() {
	_m.Called()
}
